...
class Embedding(object):
	...
	def normalize_words(self, ord=2, inplace=False, nonorm=False):
	...
	def from_glove(fname, vocab_size, dim):
		with _open(fname, 'r') as fin:
			...
			for line_no, line in enumerate(fin):
				...
				try:
					word, v=" ".join(parts[0:len(parts) - dim]), list(map(np.float32, parts[len(parts) - dim:]))
					b4=len(words)
					words_set.add(word)
					if len(words_set)>b4:
						vectors[line_no - ignored] = v
						words.append(word)
					else:
						ignored+=1
...
#					words.append(word)
			...
			return Embedding(vocabulary=vocabulary, vectors=vectors[0:len(words)])
