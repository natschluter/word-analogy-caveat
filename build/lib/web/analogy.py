...
class SimpleAnalogySolver(sklearn.base.BaseEstimator):
	...
	def __init__(self, w, method="add", batch_size=300, k=None, nonorm=True, checkc=False):
		self.w = w
	...
	def predict(self, X, nocheat):
		...
		for id_batch, batch in enumerate(batched(range(len(X)), self.batch_size)):
			...
			if not nocheat: #i.e., if you would like to cheat an take out the premise vectors
				#print 'Still cheating!'
				#Remove words that were originally in the query
				for id, row in enumerate(X_b):
					D[[w.vocabulary.word_id[r] for r in row if r in w.vocabulary.word_id], id] = np.finfo(np.float32).min

			output.append([words[id] for id in D.argmax(axis=0)])
		return np.array([item for sublist in output for item in sublist]) #out of k