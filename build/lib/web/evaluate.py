...
def evaluate_analogy(w, X, y, nocheat, method="add", k=None, category=None, batch_size=100):
	"""
	Simple method to score embedding using SimpleAnalogySolver
	...
	y_pred = solver.predict(X, nocheat)
...
def evaluate_on_WordRep(w, max_pairs=1000, solver_kwargs={}, nocheat=False):
	"""
...
def evaluate_on_all(w, nocheat):
	...
	for name, data in iteritems(analogy_tasks):
		analogy_results[name] = evaluate_analogy(w, data.X, data.y, nocheat)
